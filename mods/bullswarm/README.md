...
`claude-code:wati` shows as `claude-code:w` (first letter of the account slug,
unless two accounts would collide). The plugin option `poolAliases`
(`/config`, or `pluginConfigs` in settings) overrides any name:
`claude-code:wati=cc:w,claude-code:petsona=cc:p`. The real pool names are
unchanged everywhere else, because bullswarm derives them from the account's
config directory (`~/.claude-wati`) and the keychain entry is keyed by it.
