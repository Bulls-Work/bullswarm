...
function headerRows(page: Raw, fallback: BullswarmStep | null): StepPaneRow[] {
  ...
  const actionId = text(identity.actionId ?? action.id ?? fallback?.id, 'step')
  const runId = text(identity.shortId ?? page.shortId, '—')
  const status = statusText(identity.status ?? attempt.status ?? fallback?.status)
  const pool = text(header.pool ?? attempt.pool ?? fallback?.attempt?.pool)
  const model = text(header.model ?? attempt.model ?? fallback?.attempt?.model)
  const effort = text(header.effort ?? attempt.effort ?? record(page.route)?.effort ?? fallback?.effort)
  const durationBlock = record(header.duration) ?? {}
  const active = duration(
    page.activeDurationMs ?? header.activeDurationMs ?? durationBlock.activeMs,
    page.activeMinutes ?? durationBlock.activeMinutes,
  )
  const spanMs = page.spanDurationMs ?? header.spanDurationMs ?? durationBlock.spanMs
  const span = finite(spanMs) === null ? '' : ` · span ${duration(spanMs)}`
  const purpose = nullableText(identity.purpose ?? action.purpose ?? fallback?.purpose)
  const routeReason = nullableText(record(page.route)?.reason ?? record(attempt.routing)?.reason ?? fallback?.attempt?.routeReason)
  return [
    row('identity', 'text', `${actionId} · run ${runId}`),
    row('status', 'text', `${status} · ${verdictText(identity)} · ${pool} · ${model} · ${effort}`, statusTone(status)),
    row('duration', 'text', `active ${active}${span}`, 'dim'),
    ...(routeReason ? [row('route', 'text', `route · ${routeReason}`, 'dim')] : []),
    ...(purpose ? [row('purpose', 'text', `purpose · ${purpose}`, 'dim')] : []),
  ]
}
...
function activityRows(page: Raw, options: StepPaneOptions): StepPaneRow[] {
  ...
  const controls = row(
    'controls',
    'text',
    `filters [${text(activity.filter, 'all')}] · all · turns · errors · tools · ${activity.follow === false ? 'paused' : 'following'}`,
    'dim',
  )
  if (activity.available === false || (!activity.available && !arrayOf(activity.events).length && !arrayOf(activity.turns).length)) {
    return [controls, row('unavailable', 'text', String(activity.reason ?? 'event stream unavailable'), 'dim')]
  }
  if (mode === 'detail') {
    const events = arrayOf(activity.visibleDetailEvents ?? activity.todayEvents ?? activity.events)
    if (!events.length) return [controls, row('none', 'text', `no captured events for today · filters ${text(activity.filter, 'all')}`, 'dim')]
  ...
  if (overview.length) {
    ...
    const turns = arrayOf(activity.turns)
    const rows: StepPaneRow[] = [controls]
    overview.forEach((value, index) => {
      ...
      if (kind === 'response') {
        ...
        if (expandedTurn !== null && turnIndex === expandedTurn) {
          const turn = record(turns[expandedTurn]) ?? {}
  ...
  const turns = arrayOf(activity.turns)
  if (!turns.length) return [controls, row('none', 'text', `no response turns captured · ${arrayOf(activity.events).length} atomic events`, 'dim')]
  const rows: StepPaneRow[] = [controls]
...
function resultRows(page: Raw, fallback: BullswarmStep | null, options: StepPaneOptions): StepPaneRow[] {
  ...
  const rows: StepPaneRow[] = [
    row('outcome', 'text', `execution ${statusText(execution.status ?? fallback?.status)} · workflow ${text(workflow.status)} · verified ${verification.verdict == null ? '—' : verification.verdict ? 'true' : 'false'}`, statusTone(execution.status)),
  ]
  ...
  else rows.push(row('output-none', 'text', 'output unavailable.', 'dim'))
