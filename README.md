...
the dashboard's Home, Runs, Budget, Stats, History, Fleet, or Help pages, and
...
nine pages answer different questions:
...
| Runs | Which workflows are active or historical, plus integration status and product commands. |
...
| History | The dated workflow timeline, with legacy rows read-only and without figures. |
...
Home, Budget, Stats, and History read a per-run rollup that every finishing
...
| [Observing runs](https://bulls-work.github.io/bullswarm/guide/observing) | `workflow watch`, the Home/Runs/Run/Step/Budget/Stats/History/Fleet/Help dashboard pages, keys, mouse, and terminal glyphs |
