...
function middleCut(text, width) {
  ...
  // scoped form `claude-co…wati` rather than losing the scope to a prefix cut.
...
export function renderStackedColumnChart({
  ...
  const heading = fit(rule(title ?? '', null, cols), cols);
  if (!labels.length) return { lines: [heading, lineWithReason('—', 'no measured data', cols)], regions: [] };
  const painterSeries = sourceSeries.map((entry) => ({
    name: entry.id ?? entry.label ?? entry.name ?? null,
    values: entry.values,
    color: entry.color ?? seriesColor(entry.id ?? entry.label ?? entry.name ?? ''),
  }));
