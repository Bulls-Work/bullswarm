 ...
 * configured one. A measured rate is charged directly with no floor; this
 * setting is only the unmeasured-pool tie-breaker (and the fallback for an
 * in-flight record whose duration is unknown).
 * `config.inflightPenaltyPct: 0` disables that fallback entirely; anything
