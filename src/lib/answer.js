 ...
 * Returns `{ answer, answered, answerCheck: { ok, errors, file, why } }`.
 * `answer` is the parsed value even when it breaks the schema, and null when
 * there is nothing to parse. `answered` is true when this run wrote JSON to
 * the file, valid or not. A file that existed before the run (`mtimeBefore`)
 ...
 * so a terse reply beside an answer the worker wrote is not a failure. A
 * non-zero exit, failure words, a limit or a sign-in failure still fail the
 * worker.
 */
 ...
 * only failure was a thin reply beside an answer it wrote: then the worker did
 * its part, and the check alone decides `ok`.
 */
export function withAnswerCheck(verdict, { answer, answerCheck, answered = answerCheck.ok || answer !== null }) {
  if (answered && failedOnlyOnProse(verdict)) {
    const why = answerCheck.ok
      ? `answer valid (reply: ${verdict.why})`
      : `answer check failed (${answerCheck.why}) · reply: ${verdict.why}`;
    return { ...verdict, ok: answerCheck.ok, workerOk: true, why, answer, answerCheck };
  }
