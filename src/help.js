...
const runText = rich({
  ...
  options: [
    ...
    { flag: '--batch <tasks.jsonl>', desc: 'run each line of a JSONL file as its own single run (one attempt each, the same routing and checks) and print one JSON array of the verdicts in file order, each with the line\'s `id`, the run\'s decision-log `runId`, and `exit`. A line is {"id", "lane", "prompt" or "taskFile", optional "addDir", "effort", "reasoning", "answerSchema", "answerFile", and the route filters "avoidPool", "useProvider", "avoidProvider", "independentOf" (a value or a list)}. A bad line, an unknown key, a schema that cannot be checked, or a route filter naming no configured pool or finished run exits 2 before anything runs; exit 1 when any task failed. With --batch only --concurrency, --json, --no-caller, --timeout and --dry-run apply, to every task', default: 'off — one task' },
    ...
    { flag: '--independent-of <run>', desc: 'route filter: never use the provider that ran an earlier run, named by the `outFile` or `id` from its --json verdict (`runId` in a --batch array), so a checker never shares the writer\'s vendor. Repeatable; a run not in the decision log exits 2', default: 'none' },
...
const workflowPauseText = rich({
  ...
  options: [
    ...
    { flag: '--json', desc: 'print {status: paused|pausing, mode, appliedBy, next}', default: 'one human line' },
  ],
  safety: [
    'writes pause.json in the run directory; a run with no live kernel is marked paused immediately',
    '--now stops agents mid-step; files they already changed stay in the workspace',
...
const HELP = {
  ...
  workflow: {
    ...
    plan: {
      ...
      export: { _text: workflowPlanExportText },
