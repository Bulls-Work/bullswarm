...
test('near-limit pool soft-yields to a behind-pace headroom pool', () => {
  ...
  assert.equal(r.candidates.find((c) => c.pool === 'claude-code:wati').nearFiveHourPenalty, true);
...
test('near-limit pool is still picked when no other pool is behind pace', () => {
  ...
  assert.match(r.why, /last mile: claude-code:wati 82% of 5h, handoff covers the wall/);
  assert.doesNotMatch(r.why, /skipped near 5h limit/);
});
...
test('a near-limit pool stays selectable when the alternative is not behind pace', () => {
  ...
  assert.equal(r.pick.pool, 'claude-code:wati');
  assert.match(r.why, /last mile: claude-code:wati 91% of 5h, handoff covers the wall/);
  assert.equal(r.candidates.find((c) => c.pool === 'claude-code:wati').nearFiveHourPenalty, false);
});
...
test('an incumbent may keep a near-limit pool because the soft penalty is ordering-only', () => {
  ...
  assert.match(r.why, /last mile: claude-code:wati 79% of 5h, handoff covers the wall/);
});
...
test('candidateMinutes null forecasts the projection alone', () => {
  ...
  assert.match(r.why, /last mile: wati 78% of 5h, handoff covers the wall/);
});

test('a pool forecast past 100% remains eligible but is ranked last', () => {
  const over = pool('wati', {
    pace: 80,
    fiveHourUsedPct: 70,
  ...
  assert.equal(r.candidates.find((c) => c.pool === 'wati').forecastGated, false);
  assert.equal(r.candidates.find((c) => c.pool === 'wati').forecastOverLimit, true);
  assert.equal(r.candidates.at(-1).pool, 'wati'); // over-limit pools sort last
  assert.match(r.why, /forecast over 100% \(still eligible; ranked last\): wati 102\.6%/);
});
...
test('an unknown forecast is never gated and a known over-limit forecast is last', () => {
  ...
  const over = pool('wati', { pace: 90, fiveHourUsedPct: 88, projectedFiveHourPct: 101 });
...
test('between pools of equal pace the one carrying measured work still yields', () => {
  ...
  assert.deepEqual(
    ...
    [['codex', 10, 9.7, 0, 'history'], ['wati', 10, 7.7, 2, 'history']],
  );
 ...
 * `claude-code:wati 88.1%` and `claude-code:petsona 75.3%`.
 *
...
test('a near-limit pool under its 5h clock keeps the lane and names the handoff (R10 replay)', () => {
  ...
  assert.match(r.why, /last mile: claude-code:wati 88\.1% of 5h, handoff covers the wall/);
  assert.deepEqual(
    r.candidates.map((c) => [
      c.pool, c.pace, c.forecastFiveHourPct, c.fiveHourElapsedPct, c.nearFiveHourLimit,
    ]),
    [
      ['claude-code:wati', 21.4, 88.1, 92.3, true],
      ['claude-code:petsona', 3.4, 75.3, 85.7, true],
...
test('the same 88.1% forecast four hours from the reset gets a soft penalty', () => {
  ...
  assert.equal(wati.nearFiveHourPenalty, true);
  assert.doesNotMatch(r.why, /skipped near 5h limit/);
...
test('a 90% forecast is still eligible regardless of the clock', () => {
  ...
  assert.match(r.why, /last mile: wati 90\.5% of 5h, handoff covers the wall/);
});
...
test('past the 95% line but under its clock, an expiring pool is on pace and stays urgent', () => {
  ...
  // Both expiring pools are urgent; wati's 22.9 over 8.1% of its week (283)
  // still outranks grok's 2.8 over 1.2% (233), so grok is second, not last.
  assert.deepEqual(r.candidates.slice(0, 2).map((c) => c.pool), ['claude-code:wati', 'grok']);
  assert.equal(r.pick.pool, 'claude-code:wati');
...
test('an evidence reason names every writer pool it ranked below the winner', () => {
  ...
  const r = pickPool('analyze', [
    pool('grok', { pace: -5 }),
    pool('claude-code:wati', { meter: { type: '5h', windowStart: NOW - 4.9 * HOUR, usedPct: 5 } }),
    pool('codex', { pace: 30 }),
  ], {
    callerEligible: false, callerSession: false, now: NOW,
    evidence: { writerPools: ['codex', 'claude-code:wati'] },
  });
  assert.equal(r.pick.pool, 'grok');
  assert.ok(
  ...
  // wati ranked ahead of codex and still lost the step to the non-writer.
  assert.match(r.why, /^evidence: independent of claude-code:wati, codex \(they produced the judged work\)/);
