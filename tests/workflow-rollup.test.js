...
function stateFixture({
  ...
  return {
    ...
    attempts: attempts ?? [
      { id: 'a1', actionId: 'build', pool: 'claude-code:acme', model: 'claude-opus-5', status: 'succeeded', wallSec: 876.9, usage: { tokens: { totalKnown: 10841 }, cost: { estimatedUsd: 0.244205 } } },
...
test('R1: the record carries every field the dashboard reads, measured from state and result', () => {
  ...
  assert.deepEqual(record.pools['claude-code:acme'], { attempts: 1, minutes: 14.62, costUsd: 0.244205, tokens: 10841 });
