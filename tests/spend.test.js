...
test('every live reading is appended to the pool history log', async () => {
  ...
  try {
    ...
    const path = join(home, 'meters', 'history', 'claude-code:acme.jsonl');
...
test('spend rate is fitted over consecutive readings inside one window', () => {
  // The measured claude-code:acme series: 26% → 51% → 84% of the 5h window
  ...
  const rate = spendRateFor('claude-code:acme', {
    history,
