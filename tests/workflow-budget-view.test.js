...
test('Budget lists disabled pools once in a dim footer, not as meter rows', () => {
  const result = budgetLines({
    ...budgetFixture(),
    disabledPools: ['claude-code:petsona', 'echo'],
  }, { width: 120, ansi: true });
  const text = result.lines.join('\n');
  assert.match(text, /\x1b\[2mdisabled: claude-code:petsona, echo\x1b\[0m/);
  assert.doesNotMatch(strip(text), /claude-code:petsona\s+.*Licence meter/);
});
