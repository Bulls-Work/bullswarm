...
/** The signature of the limit notice findQuotaFailure reads in `text`, or null. */
const noticeOf = (connector, text) => findQuotaFailure(connector, text)?.signature ?? null;
...
test('reset wording alone is not a usage-limit signature', () => {
  ...
  assert.equal(noticeOf({}, 'Weekly window resets at 3pm; 26% used.'), null);
  assert.equal(noticeOf({}, 'Weekly window resets in 2 hours.'), null);
});

test('a short line leading with the signature is quota-shaped', () => {
  const connector = { quotaSignatures: ['hit your session limit'] };
  assert.equal(
    noticeOf(connector, "You've hit your session limit · resets 8:20pm (Asia/Hong_Kong)"),
    'hit your session limit',
  );
  // Error-shaped lines qualify even when the phrase starts past character 40.
  assert.equal(
    noticeOf({}, 'Error: the upstream provider replied 429 too many requests'),
    'too many requests',
  );
});
...
test('a report that merely discusses limits is not a quota failure', () => {
  ...
  assert.ok(report.includes('rate limit exceeded'), 'the words are present');
  assert.equal(noticeOf({}, report), null, 'but the phrase is buried in prose');
  ...
  assert.equal(noticeOf({}, paragraph), null);
  ...
  assert.equal(noticeOf({}, bullet), null);
});
...
test('generic phrases count only as a bare notice, never as narration about another service', () => {
  ...
  for (const narration of [
    ...
    assert.ok(DEFAULT_QUOTA_SIGNATURES.some((phrase) => narration.toLowerCase().includes(phrase)), `words are present: ${narration}`);
    assert.equal(noticeOf({}, narration), null, narration);
  }
  ...
  assert.equal(noticeOf({}, 'Rate limit exceeded · resets 3pm'), 'rate limit exceeded');
  assert.equal(noticeOf({}, '429 Too Many Requests'), 'too many requests');
  assert.equal(noticeOf({}, 'Too many requests. Please retry after 30 seconds.'), 'too many requests');
  assert.equal(noticeOf({}, 'Error: rate limited'), 'rate limited');
  assert.equal(noticeOf({}, 'quota exceeded (resets in 2 hours)'), 'quota exceeded');
  // Provider first-person wording is not generic and keeps the plain rule.
  assert.equal(
    noticeOf({}, "You've hit your usage limit for this session and cannot continue right now"),
...
test('connector-declared signatures extend the defaults', () => {
  assert.equal(noticeOf({ quotaSignatures: ['seat is spent'] }, 'the seat is spent'), 'seat is spent');
  assert.equal(noticeOf({}, 'the seat is spent'), null);
