...
const buckets = [
  ...
  { key: '2026-09-14', label: '2026-09-14', value: 2.93, tokenSource: 'estimated:utf8-bytes/4', segments: [{ name: 'wati', value: 2.93 }] },
...
function fixture({ emptyModels = false } = {}) {
  const pools = [
    ...
    { name: 'wati', runs: 2, attempts: 74, minutes: 1258, apiEquivalentUsd: 3.27, tokenSource: 'estimated:utf8-bytes/4', minutesShare: 0.179, verified: 9, verifiedShare: 0.6, live: { usedPct: null } },
  ...
  return {
    overview: {
      ...
      breakdown: { pools, models, projects: [{ name: 'agentic-knowledge-system', runs: 20, attempts: 40, minutes: 3000, apiEquivalentUsd: 2.1, tokenSource: 'estimated:utf8-bytes/4', minutesShare: 0.427 }, { name: 'bullswarm', runs: 15, attempts: 30, minutes: 1900, apiEquivalentUsd: 1.8, tokenSource: 'estimated:utf8-bytes/4', minutesShare: 0.271 }] },
    },
    ...
    projects: {
      rows: [{ name: 'agentic-knowledge-system', runs: 20, attempts: 40, minutes: 3000, apiEquivalentUsd: 2.1, tokenSource: 'estimated:utf8-bytes/4', minutesShare: 0.427 }, { name: 'bullswarm', runs: 15, attempts: 30, minutes: 1900, apiEquivalentUsd: 1.8, tokenSource: 'estimated:utf8-bytes/4', minutesShare: 0.271 }],
      trend: { metric: 'runs', segmentBasis: 'the project each run recorded', buckets: buckets.map((bucket, index) => ({ ...bucket, value: index < 4 ? index + 2 : 0, segments: index < 4 ? [{ name: 'agentic-knowledge-system', value: index + 1 }, { name: 'bullswarm', value: 1 }] : [] })) },
    },
  };
}
...
test('one Stats render shares chart hues with panel rows and keeps visible panel names distinct', () => {
  const pools = [
    ...
    { name: 'claude-code:wati', runs: 2, attempts: 2, minutes: 2, apiEquivalentUsd: 2 },
    { name: 'pool-only', runs: 1, attempts: 1, minutes: 1, apiEquivalentUsd: 1 },
  ];
  const buckets = [
    { key: '2026-09-13', label: '2026-09-13', value: 3, segments: [{ name: 'codex', value: 2 }, { name: 'claude-code:wati', value: 1 }] },
    { key: '2026-09-14', label: '2026-09-14', value: 3, segments: [{ name: 'codex', value: 1 }, { name: 'claude-code:wati', value: 2 }] },
  ];
  const stats = {
  ...
  const wati = view.lines.find((line) => visible(line).startsWith('wati '));
  const extra = view.lines.find((line) => visible(line).startsWith('pool-only '));
  assert.ok(codex && wati && extra, 'all visible pool rows are rendered');
  ...
  const watiColour = colour(wati);
  const extraColour = colour(extra);
  assert.equal(codexColour, legendColour('codex'));
  assert.equal(watiColour, legendColour('claude-code:wati'));
  assert.equal(new Set([codexColour, watiColour, extraColour]).size, 3, 'panel rows have pairwise distinct hues');
});
...
test('every visible Spending panel track is covered cell-for-cell, including tiny bars', () => {
  ...
  assert.ok(poolRows.some((region) => region.action.payload.label === 'wati'));
