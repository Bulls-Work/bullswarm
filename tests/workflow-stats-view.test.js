...
test('Trends stacks are coloured per pool and the legend wraps, never truncated', () => {
  const many = { ...trend, buckets: Array.from({ length: 7 }, (_, index) => ({
    ...
    segments: Object.entries({
      'claude-code': 1, codex: 1, grok: 1, 'command-code': 1, opencode2: 1, echo: 1, 'kaihk-3/gpt-5.6-luna': 1, 'claude-code:wati': 1,
...
test('the Trends legend wraps to show every pool instead of `+N more`', () => {
  const names = ['unknown', 'opencode2', 'opencode2:kaihk-2', 'opencode2:kaihk-3', 'command-code', 'grok', 'claude-code:wati', 'claude-code', 'codex', 'opencode', 'echo'];
  const trend = {
