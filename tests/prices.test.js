...
test('a team seat never falls through to its usage multiplier price', () => {
  // Observed on the owner's real home (2026-09-18): claude-code:acme reports
  ...
  const acme = {
    name: 'claude-code:acme',
    connector: { name: 'claude-code:acme', profile: { slug: 'acme' } },
    meterSnapshot: {
  ...
  assert.equal(planPriceFor(acme, acme), null);
