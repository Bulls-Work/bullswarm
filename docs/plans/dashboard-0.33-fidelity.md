...
| `Max 20x · $200/mo`, `$18.50 of $50 used`, `worth ≈ $212 at API rates` | `monthlyPriceUsd` is null on every pool: `no declared subscription price: claude-code, claude-code:acme, codex, grok, command-code, opencode2` |
...
- **No prototype frame exists for the Runs list.** It is not a requirement of
  ...
  vocabulary.
- **No prototype frame exists at 200 columns.** The prototype screenshots are a
...
```bash
bullswarm workflow plan validate "$(cat /home/dev/Repo/bullswork/bullswarm-dashboard/docs/plans/dashboard-0.33-fidelity.goal.txt)" \
  --cwd=/home/dev/Repo/bullswork/bullswarm-dashboard \
  --program=/home/dev/Repo/bullswork/bullswarm-dashboard/docs/plans/dashboard-0.33-fidelity.program.json --json
```
