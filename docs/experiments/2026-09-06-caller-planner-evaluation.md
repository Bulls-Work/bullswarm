...
Routing: the machine's live default strategy (no pins): high → `claude-code:initech` / `claude-opus-5`,
medium → `claude-code:initech` / `claude-sonnet-5`, low → `command-code` / `gpt-5.6-luna`; Fable
...
| Pools used | `claude-code:initech` (opus, sonnet-5), `command-code` (luna) | `claude-code:initech` (sonnet-5), `command-code` (luna) |
