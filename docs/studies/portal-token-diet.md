# project-h token diet — what transfers to bullswarm

**Verdict.** project-h's headline "90% savings" is not reproducible from project-h's own repository, and the mechanism behind it — a hook that blocks a frontier model from reading big files — has no cheap path into bullswarm today. Every bullswarm dispatch is a full agentic CLI session; read-only dispatches already run a median of 72.1 s, far past project-h's own stated floor where "the overhead of delegation exceeds the savings" (`.study-inputs/article.txt:86`). What does transfer is project-h's *policy*, not its plumbing: name the kinds of work a frontier model is worth, and keep everything else off it. Bullswarm's own 7-day log says that is where the money sits — `claude-opus-5` took 35.6% of dispatches but 75.2% of worker wall-minutes. Build L1 and L5 first. L2 (the hook) is the least justified rung on the ladder.
...
## 1. What project-h does, and what its 90% counted

project-h's `project-h-plugin` plugin has three layers (`.study-inputs/project-h/plugins/project-h-plugin/`):

1. **Two PreToolUse hooks.** `hooks/check-file-size` blocks a `Read` when the file exceeds `MIN_LINES="${PROJECT_H_MIN_LINES:-350}"` (`check-file-size:5`), allowing it through when `offset`/`limit` is set. `hooks/check-bash-read` applies the same rule to `cat/head/tail/less/more`.
2. **Two one-shot scripts** over a single stateless `project-h-llm:invoke-chat` call: `bulk-read` (files + a question) and `code-write` (spec + mandatory reference). Transport caps: `PROJECT_H_TIMEOUT_SECONDS=180` and `PROJECT_H_MAX_PAYLOAD_BYTES` 400000 on macOS / 120000 on Linux (`scripts/lib/project-h-llm.sh:22-30`).
...
- **project-h limit:** none engaged — this is guidance, not delegation.

### L2 — Caller-side project-h-plugin hook, bullswarm as bulk-reader / code-writer backend
...
- **Expected saving:** **none substantiated.** project-h's own 90% is unreproducible, and bullswarm's I/O-like median is already 72.1 s (map-policy classification over the 188 rows) — above project-h's own "counterproductive for small ones" line. Do not budget a number here.
...
- **project-h limit:** hits all three — no line numbers, no delegated reasoning, and the latency floor is the disqualifier.

### L3 — Worker-side hooks and pre-digest actions in the kernel
- **Mechanism:** two halves. *Refuted half:* inject a project-h-plugin hook into dispatched Claude Code workers. *Kept half:* generalize `preflight-scout` into a reusable low-effort **digest** action that reads a worker's `dependencyArtifacts()` (`v2-runtime.js:264-269`) so the consumer is handed a condensed artifact instead of raw out-files.
...
- **project-h limit:** "You can't delegate reasoning" — hence extractive-only.
...
- **project-h limit:** the same summary-loses-detail failure project-h names for its thread-safety bug.
...
- **project-h limit:** this is project-h's rule inverted — project-h excludes architecture from the cheap worker; bullswarm reserves the expensive worker for it.
...
| Idea | Why it fails |
|---|---|
| Caller-side or worker-side project-h-plugin hook redirecting large reads to bullswarm | No bounded non-agentic call mode; I/O-like median is already 72.1 s, past project-h's own latency floor. The worker-side variant also burns the only nested-dispatch headroom (`depthLimit: 2`) and cannot be tested — real pools are forbidden in tests. |
...
| Keep architecture work on the calling session, project-h-style | Inverts `R5` (`src/lib/route.js:13`): "The caller wins its lane only when no eligible delegate remains — it has to WIN, not be protected." Also shifts spend onto the user's own session rather than saving it. |
...
**Not planned.** L2 (caller-side project-h-plugin hook) is not planned. §7 item 7 still holds: do not build it for this.
