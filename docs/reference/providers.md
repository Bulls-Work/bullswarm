...
| `grok` | plain `grok-N.M` (no suffix): high, 5 · `grok-N.M-build-fast`: high, 5, never auto-recommended |

Grok has one model line, so one family covers every plain version and the newest wins: `grok-4.7` today, and a `grok-4.8` the day `grok models` lists it. Medium and low run it too, at lighter reasoning (see [below](#newest-generation-fallback-generationfallback)). `grok-4.7-build-fast` is Grok Build's id for Grok 4.7 Fast, which [xAI describes](https://docs.x.ai/developers/grok-4-7) as "the same model served on faster infrastructure, billed at twice the standard token rates". It keeps its base model's rank, is never suggested, and stays selectable with `strategy set-rung`. It has no price row: xAI's [Pricing page](https://docs.x.ai/developers/pricing#grok-47-fast-pricing-cursor-and-grok-build-only) lists its long-context rates as $6.00 / $1.50 / $18.00 per 1M tokens (checked 2026-09-23), which is not twice grok-4.7's $4.00 / $1.00 / $12.00. The contrib providers keep their own rows.
...
- **No family for the tier.** An opted-in tier that no family serves, and that no exact row ranks a model for, takes the newest model of the best-ranked family that has one in the newest generation. This is how a provider with one model line runs the same model on every tier. A model the connector opts out of (`autoRecommend: false`) never stands in.
- **Why.** A tier may declare `why`, a plain reason that replaces the generated one. Without it, a stale tier reads `no gpt-6 terra yet, newest generation preferred` and a tier with no family reads `no <tier> family, newest <family> preferred`.
...
- **Standing.** The stand-in takes the place of the candidate it replaces, normally the stale family's newest model. Other pools are compared with it exactly as before, and the stale model is still listed directly below it with the reason. A tier with no family has no rank there, so its stand-in stands at rank 0: it is always that pool's rung, and it is the tier-wide best pick only when no other pool serves the tier.
...
Grok opts medium and low in, also an owner decision (2026-09-23). Grok has one model line, so every tier suggests the newest `grok-N.M`, and the tiers differ only by reasoning. No family serves medium or low, so both take `grok-4.7` today, at the connector's own tier defaults: `high` for medium and `medium` for low. High is the family's own tier and runs at its default, `xhigh`. `strategy show` names the rung under each tier, for example `grok rung: grok-4.7 · medium reasoning — one Grok line, lighter reasoning for lighter tiers`. The tier-wide best pick and pace-based routing across pools do not change.

### `unranked`

A discovered model that matches no family and no row with a `tier` is `ranking: "unranked"` in `discoveries`. It is also listed in the report's top-level `unranked` array (`pool`, `model`, `reason`). `strategy show` and the setup review screen count these models per pool on one line (`unranked: 310 models (command-code 180, opencode 130) · never recommended · strategy show --json lists them`), because a pool with no family rules can list hundreds; `strategy show --json` keeps the full list, and the model pickers label each one `(unranked)`. It is never dropped, and because it has no tier it is never auto-recommended. To use it, select it for a tier explicitly (`strategy set-model` or `set-rung`), or add a family rule or row.
